...
use crate::metrics;
use model::block::{Approval, ApprovalInner};
use model::clock::{Clock, Duration, Instant, Utc};
use model::debug::{ApprovalAtHeightStatus, ApprovalHistoryEntry};
use model::hash::CryptoHash;
use model::signature::Signature;
use model::types::{AccountId, ApprovalStake, Balance, BlockHeight, BlockHeightDelta};
use model::validator_signer::ValidatorSigner;
...
mod trackable {
    use helper::metrics::IntGauge;
    use model::types::BlockHeight;
    ...
    pub fn process_timer(&mut self, signer: &Option<Arc<ValidatorSigner>>) -> Vec<Approval> {
        ...
        for _ in 0..MAX_TIMER_ITERS {
            ...
            if now >= self.timer.started + skip_delay {
                ...
                self.largest_target_height.set(std::cmp::max(
                    self.timer.height + 1,
                    self.largest_target_height.get(),
                ));
...
mod tests {
    ...
    use crate::Doomslug;
    use model::block::{Approval, ApprovalInner};
    use model::clock::{Duration, FakeClock, Utc};
    use model::hash::hash;
    use model::signature::{KeyType, SecretKey};
    use model::test_utils::create_test_signer;
    use model::types::ApprovalStake;
    use std::sync::Arc;
    ...
    fn test_doomslug_approvals() {
        ...
        let stakes = accounts
            .iter()
            .map(
                |(account_id, stake_this_epoch, stake_next_epoch)| ApprovalStake {
